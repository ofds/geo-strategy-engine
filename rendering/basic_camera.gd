...
func _ready() -> void:
	...
	var chunk_size_m: float = 15360.0
	
	# Calculate center of terrain in world space
	var center_chunk_x: float = 15.0
	var center_chunk_y: float = 8.0
	var grid_center_world_x: float = (center_chunk_x + 0.5) * chunk_size_m
	var grid_center_world_z: float = (center_chunk_y + 0.5) * chunk_size_m
